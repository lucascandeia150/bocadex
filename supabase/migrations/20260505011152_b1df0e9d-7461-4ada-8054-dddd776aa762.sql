...
-- 1) Prevent anonymous users from reading sensitive partner columns
REVOKE SELECT (access_pin, owner_name) ON public.partner_applications FROM anon;

-- 2) Require auth on customer_create_delivery and revoke anon execute
...
REVOKE EXECUTE ON FUNCTION public.customer_create_delivery(uuid, text, text, text, text, numeric) FROM anon, PUBLIC;
GRANT EXECUTE ON FUNCTION public.customer_create_delivery(uuid, text, text, text, text, numeric) TO authenticated;
