      ...
      <div className="p-3">
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <button onClick={onAdd} aria-label={`Adicionar ${p.name}`}
            ...
            <Plus ack bg-[hsl(142,70%,45%)] text-white px-2 py-0.5 rounded-full uppercase">
              <Clock size={9}/> Aberto
            </span>
          </div>
        </div>
...
function ProductRow({ p, onAdd }: { p: Product; onAdd: () => void }) {
  const promo = isPromo(p);
  return (
    <article className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden flex">
      <div className="flex-1 p-3.5 min-w-0">
        <div className="flex items-center gap-1.5 flex-wrap">
          {p.is_featured && <span className="text-[9px] font-black bg-secondary/20 text-secondary px-1.5 py-0.5 rounded">⭐</span>}
          {promo && <span className="text-[9px] font-black bg-destructive/15 text-destructive px-1.5 py-0.5 rounded">PROMO</span>}
        </div>
        <h3 className="text-base font-black text-foreground leading-tight mt-0.5">{p.name}</h3>
        {p.description && <p className="text-xs text-muted-foreground mt-1 line-clamp-2">{p.description}</p>}
        <div className="flex items-baseline gap-2 mt-2">
          {promo && <span className="text-xs line-through text-muted-foreground">R${Number(p.original_price).toFixed(2)}</span>}
          <span className={`text-base font-black ${promo ? "text-destructive" : "text-primary"}`}>{priceLabel(p)}</span>
        </div>
      </div>
      <div className="relative w-28 shrink-0 bg-gradient-to-br from-[hsl(142,60%,55%)] to-[hsl(24,90%,55%)]">
        ...
        <button onClick={onAdd} aria-label={`Adicionar ${p.name}`}
          className="absolute bottom-2 right-2 w-9 h-9 rounded-full bg-[hsl(142,70%,45%)] text-white flex items-center justify-center shadow-lg active:scale-90">
